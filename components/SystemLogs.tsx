...
import { Terminal, XCircle, CheckCircle, AlertTriangle, Info, Filter, Trash2, Clock, Check, Search, Globe } from 'lucide-react';
...
export const SystemLogs: React.FC<Props> = ({ logs, onClear, retentionHours = 168, onRetentionChange }) => {
  ...
  }, [logs, filter, ipSearch]);

  const filteredLogs = logs.filter(log => {
    const matchesType = filter === 'all' || log.type === filter;
    const matchesIp = ipSearch === '' || 
                     (log.ip && log.ip.includes(ipSearch)) || 
                     log.event.includes(ipSearch);
  ...
  return (
    <div className="bg-slate-950 rounded-2xl p-4 border border-slate-800 h-[400px] flex flex-col shadow-inner font-mono relative overflow-hidden group">
       ...
       {/* Header */}
       <div className="flex flex-col gap-3 mb-3 border-b border-slate-800 pb-2">
         <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
                <button 
                    ...
                    className={`p-1.5 rounded transition-colors ${showSearch || ipSearch ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                >
                ...
                {onRetentionChange && (
                    <div className="relative">
                        ...
                        {showRetentionMenu && <div className="fixed inset-0 z-10" onClick={() => setShowRetentionMenu(false)}></div>}
                    </div>
                )}
         ...
         {(showSearch || ipSearch) && (
            <div className="relative animate-in slide-in-from-top-1 duration-200">
                <input 
                    ...
                    placeholder="FILTRER PAR IP OU TEXTE..."
                    className="w-full bg-slate-900 border border-slate-800 rounded-lg py-1.5 pl-8 pr-3 text-[10px] text-brand-500 outline-none focus:border-brand-500/50 transition-all uppercase placeholder:text-slate-700 font-bold"
                />
       ...
       <div className="flex-1 overflow-y-auto space-y-1.5 custom-scrollbar p-1">
          {filteredLogs.map((log) => (
            <div key={log.id} className="flex gap-2.5 text-[10px] md:text-xs hover:bg-white/5 p-1 rounded transition-colors group/item items-start">
              <div className="flex flex-col shrink-0 font-mono text-slate-500">
                <span className="pt-0.5">[{log.timestamp}]</span>
                {log.ip && (
                    <span className="text-[8px] font-black text-slate-600 flex items-center gap-1 group-hover/item:text-brand-500/60 transition-colors">
                        <Globe className="w-2 h-2" /> {log.ip}
                    </span>
                )}
              </div>
          ...
          {filteredLogs.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-slate-600 space-y-2 opacity-50">
                  ...
                  <span className="text-xs italic">Aucun résultat pour "{ipSearch || filter.toUpperCase()}"</span>
              </div>
          )}

...
import { Coins, TrendingUp, Lock, Wallet, Zap, ArrowUpRight, History, CheckCircle, CreditCard, ChevronRight, Gauge, Sparkles, Activity, ShieldCheck } from 'lucide-react';
...
export const EarningsCard: React.FC<Props> = ({ isConnected, plan, balance, reputation = 100, onUpgrade, onWithdraw, transactions = [], settings }) => {
  const isPremium = plan !== 'free';
  const isEarning = isConnected && isPremium;

  const getRate = () => {
      if (!isPremium) return "0.000";
  ...
  const getEfficiencyScore = () => {
    ...
    let score = settings.miningIntensity;
    if (settings.yieldOptimizationIA) score += 20;
    if (settings.contributionType === 'exit') score += 20;
    if (settings.contributionType === 'relay') score += 10;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 relative overflow-hidden group flex flex-col h-full shadow-sm">
      {/* Background Glow */}
      <div className={`absolute -top-10 -right-10 w-32 h-32 blur-3xl rounded-full transition-colors ${isEarning ? 'bg-amber-500/10' : 'bg-slate-500/5'}`}></div>
      ...
      <div className="relative z-10 mb-8">
        ...
        {isPremium ? (
          <div className="space-y-6">
             ...
             <div className="space-y-2">
                <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                    <span className="flex items-center gap-1.5"><ShieldCheck className="w-3 h-3" /> Santé du Nœud (Réputation)</span>
          ...
          <div className="mt-4 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center text-center">
            ...
            <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-2">Gains Désactivés</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
              Monétisez votre connexion en devenant un nœud du réseau Renumerate.
            </p>
            <button 
              onClick={onUpgrade}
